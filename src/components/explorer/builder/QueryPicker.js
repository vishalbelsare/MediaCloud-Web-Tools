  ...
  render() {
    ...
    if (queries && queries.length > 0 && selected) {
      ...
      if (isEditable) {
        ...
        const defaultQuery = { index: newIndex, label: 'enter query', q: '', description: 'new', startDate: dateObj.start, endDate: dateObj.end, collections: [{ tags_id: DEFAULT_COLLECTION, label: 'U.S. Mainstream Media' }], sources: [], color: genDefColor, custom: true };
