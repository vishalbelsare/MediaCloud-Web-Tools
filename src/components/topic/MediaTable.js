          ...
          <tbody>
            ...
            {media.map((m, idx) =>
              (<tr key={m.media_id} className={(idx % 2 === 0) ? 'even' : 'odd'}>
                ...
                <td>{m.metadata && m.metadata.media_type ? m.metadata.media_type.label : '?'}</td>
                <td>{m.metadata && m.metadata.language ? m.metadata.language.label : '?'}</td>
                <td>{m.metadata && m.metadata.pub_country ? m.metadata.pub_country.label : '?'}</td>
                <td>{m.metadata && m.metadata.pub_state ? m.metadata.pub_state.label : '?'}</td>
                <td>{m.metadata && m.metadata.about_country ? m.metadata.about_country.label : '?'}</td>
              </tr>)
            )}
          </tbody>
        </table>
