...
def _sentence_count(topic_id, snapshot_id, timespan_id):
    topic_timespan = mc.controversyDumpTimeSlice(timespan_id)
    response = mc.topicSentenceCount(topic_id, snapshot_id=snapshot_id, timespan_id=timespan_id,
        split=True, split_start_date=topic_timespan['start_date'][:10],
        split_end_date=topic_timespan['end_date'][:10])
    return response
