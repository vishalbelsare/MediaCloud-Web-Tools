...
def collection_media_list(whichUser, tags_id):
    more_media = True
    all_media = []
    max_media_id = 0
    while more_media:
        logger.debug("last_media_id %s", str(max_media_id))
        media = collection_media_list_page(whichUser, tags_id, max_media_id)
...
def collection_media_list_page(whichUser, tags_id, max_media_id):
    '''
    ...
    '''
    # TODO - this needs to do either user_mc or mc...
    return whichUser.mediaList(tags_id=tags_id, last_media_id=max_media_id, rows=100)
